...
- Use `cargo clippy -- -D warnings` — no warnings tolerated
- Use `cargo fmt` — consistent formatting enforced in CI
- Never use `.unwrap()` or `.expect()` in production code
- Use `thiserror` or project-specific error types for all error paths
- Prefer owned types over references where lifetime complexity grows
- Mark all public APIs with `#[must_use]` where appropriate
