...
async fn test_sentinel_clean_code_passes() {
    ...
    assert!(
        s.evaluate_payload("fn add(a: i32, b: i32) -> i32 { a + b }".into())
            .await
            .is_ok()
    );
}
...
async fn test_sentinel_clone_box_hook() {
    ...
    assert!(
        cloned
            .before_tool(&action, Arc::new(Mutex::new(AgentState::default())))
            .await
            .is_ok()
    );
}
...
async fn test_sentinel_before_tool_callback() {
    ...
    assert!(
        s.before_tool(&action, Arc::new(Mutex::new(AgentState::default())))
            .await
            .is_ok()
    );
}
