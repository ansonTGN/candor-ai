...
use candor_sentinel::rules::{check_conventional_commit, enforce_deterministic_rules};
...
fn test_scope_lock_enforced() {
    let check = enforce_deterministic_rules("delete database", &["read_file".into()]);
    assert!(!check.passed);
...
fn test_scope_lock_passes_when_in_scope() {
    let check = enforce_deterministic_rules("read_file test.txt", &["read_file".into()]);
    assert!(check.passed);
}
...
fn test_doctrine_vague_claim_rejected() {
    let action = AgentAction {
        id: "v".into(),
        action_type: ActionType::GenerateCode,
        payload: "this should work probably".into(),
        target_path: None,
        is_reversible: true,
        scope_tags: vec![],
        phase: "build".into(),
...
fn test_doctrine_marketing_rejected() {
    let action = AgentAction {
        id: "m".into(),
        action_type: ActionType::GenerateCode,
        payload: "revolutionary new feature".into(),
        target_path: None,
        is_reversible: true,
        scope_tags: vec![],
        phase: "build".into(),
